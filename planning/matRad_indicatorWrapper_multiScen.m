function [dvh,qi] = matRad_indicatorWrapper_multiScen(cst,pln,resultGUI,refGy,refVol,param)
...
dvh = cell(1,2);
for scen = 1:2   
    if isfield(resultGUI{1,scen},'RBExD')
        doseCube = resultGUI{1,scen}.RBExD;
    else
        doseCube = resultGUI{1,scen}.physicalDose;
    end
    ...
    dvh{1,scen} = matRad_calcDVH(cst,doseCube,'cum');
    qi  = matRad_calcQualityIndicators(cst,pln,doseCube,refGy,refVol,param);
end

figure,set(gcf,'Color',[1 1 1]);
subplot(2,1,1)
matRad_showDVH_multiScen(dvh,cst,pln);
subplot(2,1,2)
